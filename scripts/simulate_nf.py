...
#==============================================================================
pbar = ProgressBar(widgets=[Percentage(), Bar()], maxval=len(grain_params)).start()
#image_stack = [sparse.coo_matrix((nrows, ncols), dtype=bool) for i in range(nframes)]
...
for i in range(len(grain_params)):
    ...
    j_pix = cellIndices(x_col_edges, valid_xy[:, 0])
    i_pix = cellIndices(y_row_edges, valid_xy[:, 1])
    k_frame = cellIndices(ome_edges, valid_ang[:, 2])
...
# %% EXPORT
#==============================================================================
#fid = open('/Users/bernier2/Desktop/gold_cubes.ge', 'wb')
#fid.seek(8192)
#fid.write(np.frombuffer(image_stack.flatten(), dtype=bool))
#fid.close()
